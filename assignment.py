...
gdp_bars = axes[0, 1].barh(bar_positions, df_selected_countries.groupby('country')['gdp_per_capital'].mean(),
                           height=bar_height, color='#6A0572', label='GDP per Capita')
...
# Remove numerical labels inside the bars for Total Suicides
for bar, gdp in zip(gdp_bars, df_selected_countries.groupby('country')['gdp_per_capital'].mean()):
    axes[0, 1].text(bar.get_width(), bar.get_y() + bar.get_height() / 2, f'GDP: {gdp:.2f}', ha='left', va='center', color='white', fontsize=10)

# Plot 2: Grouped Bar chart - Suicides for Male, Female, and Total Population Comparison (5-year gap)
df_age_5year = df_selected_countries[(df_selected_countries['age'] == '35-54 years') & (df_selected_countries['year'] % 5 == 0)]

sns.barplot(x='year', y='suicides_no', hue='sex', data=df_age_5year, ax=axes[1, 1], palette=['#6A0572','#F9C74F'], errorbar=None)
...
axes[1, 0].pie(age_distribution, labels=age_distribution.index, autopct='%2.1f%%', startangle=90, colors=colors, explode=explode, wedgeprops=wedgeprops, shadow=True, radius=1.2)
...
sns.lineplot(x='year', y='suicides_no', data=df_selected_countries, estimator='sum', ax=ax1, color='#4B0082', marker='o', label='Suicides_no', errorbar=None)

# Secondary y-axis for HDI_for_year
ax2 = ax1.twinx()
sns.lineplot(x='year', y='HDI for year', data=df_selected_countries, estimator='mean', ax=ax2, color='#D2691E', marker='s', label='HDI for Year', errorbar=None)
...
To conclude, these detailed findings offer a strong basis for creating specific interventions and policies to address mental health challenges and lower suicide rates, especially within particular demographic groups. Understanding how economic, developmental, and demographic factors interact provides valuable insights for shaping effective strategies in mental health and well-being initiatives.
